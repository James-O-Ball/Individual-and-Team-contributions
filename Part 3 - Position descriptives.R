...
position_breakdown <- all_stats %>%
  group_by(match_id, Position) %>%  # Count positions within each game
  summarise(n = n(), .groups = 'drop') %>%  # Remove grouping
  group_by(Position) %>%            # Now sum across all games
  summarise(total = sum(n)) %>%
  arrange(desc(total))  # Optional: order by biggest first
