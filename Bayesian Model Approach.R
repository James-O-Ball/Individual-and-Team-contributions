...
###### HOME TEAM ANALYSIS ##################
# Load match data
final_data_with_subs <- read.csv("Dataset/final_data_with_subs.csv", header = TRUE, stringsAsFactors = FALSE)
...
df_replaced <- df_long %>%
  select(MatchURL, Player, Rating2) %>%
  pivot_wider(names_from = Player, values_from = Rating2) %>%
  rename_with(~ str_replace(., "_ws_rating2", "_ws_rating"), -MatchURL)
...
data1$outcome <- factor(data1$outcome, levels = c("draw", "loss", "win"))
data2$outcome <- factor(data2$outcome, levels = c("draw", "loss", "win"))
# Model Analysis ----------------------------------------------------------
library(brms)
...
true_class <- as.numeric(data1$outcome)
...
# --- 3. Run summaries ---
summary1 <- summarise_across_draws(preds1, levels(data1$outcome))
summary2 <- summarise_across_draws(preds2, levels(data1$outcome))
diff_summary1 <- summarise_across_draws(diff_preds, levels(data1$outcome)) %>%
...
df_long_opp <- final_data_with_subs %>% 
  ...
  mutate(
    ...
    Rating2 = ifelse(Flag == 'Star', mean_opp, Rating)  
  ) %>%
  ungroup()
...
# --- 2. Model Setup for Away Team ---
data3 <- dat_replaced_opp %>%  
  mutate(
    Team_ws_rating = rowMeans(select(., starts_with("Player") & ends_with("ws_rating")), na.rm = TRUE),
    Opp_ws_rating = rowMeans(select(., starts_with("Opp") & ends_with("ws_rating")), na.rm = TRUE)
  )

data4 <- dat_replaced_opp %>%  
  mutate(
    Team_ws_rating = rowMeans(select(., starts_with("Player") & ends_with("ws_rating")), na.rm = TRUE),
    Opp_ws_rating = rowMeans(select(., starts_with("Opp") & ends_with("ws_rating")), na.rm = TRUE)
  )
...
data3$away_outcome <- factor(data3$away_outcome, levels = c("draw", "loss", "win"))
data4$away_outcome <- factor(data4$away_outcome, levels = c("draw", "loss", "win"))

# --- 3. Modeling with brms ---
model3 <- brm(
...
summary3 <- summarise_across_draws(preds3, levels(data3$away_outcome))
summary4 <- summarise_across_draws(preds4, levels(data3$away_outcome))
diff_summary2 <- summarise_across_draws(diff_preds, levels(data3$away_outcome)) %>%
  rename(Diff_Mean = Mean, Diff_Lower = Lower, Diff_Upper = Upper)
