...
# Calculate season avg substitute rating per team
subs_long <- final_data_all_players %>%
  select(match_id, hometeam, awayteam, matches("_Rating$")) %>%
  pivot_longer(cols = matches("_Rating$"),
               names_to = "Player", values_to = "Rating") %>%
  mutate(is_home = grepl("^Home_", Player),
         team = if_else(is_home, hometeam, awayteam),
         # Identify subs: players 12-16
         is_sub = grepl("Player1[2-6]|Player[3-9][0-9]", Player)) %>%
  filter(is_sub) %>%
  group_by(team) %>%
  summarise(avg_sub_rating = mean(Rating, na.rm = TRUE), .groups = "drop")

# Convert to long for standout detection
df_long <- final_data_all_players %>%
  select(match_id, league, Date, hometeam, awayteam, Home_Score, Away_Score,
         matches("_Rating$")) %>%
  pivot_longer(cols = matches("_Rating$"),
               names_to = "Player", values_to = "Rating") %>%
  mutate(is_home = grepl("^Home_", Player),
  ...
  group_by(match_id) %>%
  mutate(
    group_mean = mean(Rating[is_home], na.rm = TRUE),
    group_sd   = sd(Rating[is_home], na.rm = TRUE),
    opp_mean   = mean(Rating[!is_home], na.rm = TRUE),
    opp_sd     = sd(Rating[!is_home], na.rm = TRUE),
    Flag = case_when(
      is_home & Rating > group_mean + 2 * group_sd ~ "Star",
      !is_home & Rating > opp_mean + 2 * opp_sd ~ "Star",
      TRUE ~ "Average"
    )
  ) %>%
  ungroup()

# Join avg sub ratings for replacement
df_long <- df_long %>%
  left_join(subs_long, by = "team") %>%
  mutate(
    Replacement_Rating = ifelse(Flag == "Star", avg_sub_rating, Rating)
  )


# Create replaced ratings wide format -------------------------------------
...
# Replacement long dataset ------------------------------------------------

# Home perspective
home_replaced <- final_data_all_players %>%
  ...
  transmute(
    ...
    home_advantage = 1,
    goal_diff = Home_Score - Away_Score,
    Team_Rating = rowMeans(select(., starts_with("Home_Player"), ends_with("Rating")), na.rm = TRUE),
    Opp_Rating = rowMeans(select(., starts_with("Away_Player"), ends_with("Rating")), na.rm = TRUE)
  )

# Away perspective
away_replaced <- final_data_all_players %>%
  select(match_id, league, hometeam, awayteam, Home_Score, Away_Score) %>%
  left_join(df_replaced, by = "match_id") %>%
  transmute(
    match_id,
    ...
    home_advantage = 0,
    goal_diff = Away_Score - Home_Score,
    Team_Rating = rowMeans(select(., starts_with("Away_Player"), ends_with("Rating")), na.rm = TRUE),
    Opp_Rating = rowMeans(select(., starts_with("Home_Player"), ends_with("Rating")), na.rm = TRUE)
  )
